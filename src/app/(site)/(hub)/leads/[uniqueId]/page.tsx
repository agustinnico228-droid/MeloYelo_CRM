    ...
    <main className="mx-auto max-w-3xl space-y-6 p-4 sm:p-6">
      <header className="rounded-card border border-my-line bg-my-surface p-5 shadow-card">
        ...
        <div className="mt-4 flex flex-col gap-2 sm:flex-row">
          ...
          {lead.email ? (
            <a href={`mailto:${lead.email}`} className="btn-quiet flex-1">
      ...
      {viewingAs ? (
        <section className="rounded-card border border-my-line bg-my-paper p-5 shadow-card">
          <p className="text-sm text-my-slate">
            Read-only while viewing as {viewingAs.name}. Stage changes, notes
            and edits are disabled. Exit view-as (banner above) to make changes
            as yourself.
          </p>
        </section>
      ) : (
