  ...
  return (
    <div
      ...
      className={`rounded-card border p-4 shadow-card ${
        highlight
          ? "border-my-yellow bg-my-yellow/20"
          : "border-my-line bg-my-surface"
      }`}
    >
    ...
    return (
      <main className="mx-auto max-w-2xl p-6">
        ...
        <p className="mt-4 rounded-card border border-my-line bg-my-surface p-6 text-my-slate shadow-card">
          This dashboard shows every agent&apos;s numbers and is for managers.
          Your own pipeline is on Today and Dashboards.
        </p>
      </main>
    );
  }
    ...
    <main className="mx-auto max-w-5xl space-y-6 p-4 sm:p-6">
      ...
      <section>
        ...
        <p className="mt-2 text-sm text-my-slate">
          The old dashboard counts only leads sitting at Test Ride Booked right
          now. The moment a ride completes, the lead leaves that figure, so it
          understates the real booking rate. The &quot;ever&quot; number counts
          booking events from Stage history and is the commercially meaningful
          one.
        </p>
      </section>
      ...
      <section>
        ...
        <div className="mt-3 max-w-md rounded-card border border-my-warn/60 bg-my-surface p-4 shadow-card">
          ...
          <p className="text-sm text-my-slate">
            ...
            {stl.excludedCount > 0
              ? ` - ${stl.excludedCount} outliers excluded`
              : ""}
          </p>
          <p className="mt-2 text-sm text-my-slate">
            Greg&apos;s overview confirms the definition: minutes from when the
            lead was created until it&apos;s first touched (the agent or Ride
            Guide changes the stage), which is exactly what the stored column
            records. The old Looker tile most likely shows the raw average of
            that column, which the 200,000-minute artefacts drag up; this tile
            shows the median with those excluded. Confirm the aggregation with
            Greg before treating either as authoritative.
          </p>
        </div>
      </section>
      ...
      <section>
        ...
        <p className="mt-2 text-sm text-my-slate">
          Medians exclude values over 14 days as data artefacts. The old Looker
          leaderboard includes them, which is how it shows agents at 125+ days.
        </p>
      </section>
    </main>
  );
