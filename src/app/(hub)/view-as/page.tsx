  ...
  return (
    <main className="mx-auto max-w-2xl space-y-4 p-4 sm:p-6">
      ...
      <p className="text-sm text-my-slate">
        See exactly what an agent sees — their leads, their Today page,
        nothing else. While active the hub is <strong>read-only</strong> and
        both entry and exit are recorded in the audit log. This is how you
        answer &quot;an agent says they can&apos;t see their lead&quot;
        without weakening any filter or signing in as anyone.
      </p>

      {active ? (
        <p className="rounded-card border border-my-warn/40 bg-my-warn/10 p-4 text-sm">
          Currently viewing as <strong>{active.name}</strong>. Use the banner
          at the top of any page to exit.
        </p>
      ) : null}
