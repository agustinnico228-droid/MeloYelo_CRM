  ...
  return (
    <div className="rounded-card border border-my-line bg-my-surface p-4 shadow-card">
      ...
      <p className="text-sm text-my-slate">
        median of {sample}
        {excluded > 0 ? ` · ${excluded} outlier${excluded === 1 ? "" : "s"} excluded` : ""}
      </p>
    </div>
  );
}
  ...
  const embeds =
    cmsDashboards.length > 0
      ...
      : // No CMS yet: fall back to env-configured reports
        [
          { slug: "analytics", title: "Website & Email Analytics", description: "", connected: Boolean(legacyUrls.agent) },
          ...(isManager
            ? [
                { slug: "manager", title: "Managers Dashboard", description: "", connected: Boolean(legacyUrls.manager) },
                { slug: "growth", title: "Growth Dashboard", description: "", connected: Boolean(legacyUrls.growth) },
              ]
            : []),
        ];
    ...
    <main className="mx-auto max-w-5xl space-y-6 p-4 sm:p-6">
      ...
      <section>
        ...
        <p className="mt-2 text-sm text-my-slate">
          Values over 14 days are excluded as data artefacts, so these
          medians can differ from the old Looker tiles, which include them.
        </p>
      </section>
      ...
      <section>
        ...
        <p className="mt-3 text-sm text-my-slate">
          Your day-to-day numbers live on Today and Leads — this hub
          replaces the old agent Looker view.{" "}
